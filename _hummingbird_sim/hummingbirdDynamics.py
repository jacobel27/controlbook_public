...
class hummingbirdDynamics:
    ...
    def f(self, state, u):
        ...
        N33 = 2.0*(self.J1x+self.J2x - self.m1*self.ell1**2 - self.m2*self.ell2**2 - self.J2z - self.J1y*np.sin(phi)**2 - self.J1z*np.cos(phi)**2)*np.sin(theta)*np.cos(theta)
        
        C = np.array([[(self.J1y-self.J1z)*np.sin(phi)*np.cos(phi)*(thetadot**2-np.cos(theta)**2*psidot**2)
                       +((self.J1y-self.J1z)*(np.cos(phi)**2-np.sin(phi)**2) - self.J1x)*np.cos(theta)*thetadot*psidot],
                      
                      [2.0*(self.J1z-self.J1y)*np.sin(phi)*np.cos(phi)*phidot*thetadot +
                       ((self.J1y-self.J1z)*(np.cos(phi)**2-np.sin(phi)**2) + self.J1x)*np.cos(theta)*phidot*psidot - (0.5*N33*psidot**2)],
